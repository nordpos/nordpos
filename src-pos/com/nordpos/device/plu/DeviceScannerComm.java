...
public class DeviceScannerComm implements DevicePLU, SerialPortEventListener {
...
//            e.printStackTrace();
            ...
            m_out = null;
            m_in = null;
            throw new DevicePLUException(e);
        }
    ...
    private void readCommand(byte[] cmd) throws DevicePLUException {
        byte[] b = readLine();
        if (!checkCommand(cmd, b)) {
            // excepcion que te crio.
            throw new DevicePLUException("Command not expected");
        }
    }

    private void writeLine(byte[] aline) throws DevicePLUException {

        if (m_CommPortPrinter == null) {
            throw new DevicePLUException("No Serial port opened");
        } else {
    ...
    private byte[] readLine() throws DevicePLUException {
        ...
        synchronized (this) {
            ...
            if (m_aLines.isEmpty()) {
                throw new DevicePLUException("Timeout");
            } else {
                return m_aLines.poll();
            }
        }
